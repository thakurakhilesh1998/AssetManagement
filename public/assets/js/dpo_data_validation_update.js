$(document).ready(function()
{
     ...
     $('#editAsset').submit(function(event)
     {
         if(!validateForm())
         {
...
function validateForm()
{
    ...
    else
    {
     ...
     // File input validation
        let jamabandiFiles = $('#jamabandi')[0].files; // Get the selected file
        if(jamabandiFiles.length > 0)
        {
            let jamabandiFile=jamabandiFiles[0];
    ...
    // Validate Image 
     let pictureInputs=$('#picture')[0].files;
     if(pictureInputs.length>0)
     {
        let pictureInput=pictureInputs[0];
